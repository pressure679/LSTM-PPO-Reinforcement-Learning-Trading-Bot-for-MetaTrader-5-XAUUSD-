...
def EQH(df, tolerance=1):
    ...
    prev_wick_high = df["High"].where(swing_high).ffill().shift(1)
    prev_body_high = body_high.where(body_swing_high).ffill().shift(1)

    return (
        (swing_high | body_swing_high) &
        (
            (abs(df["High"] - prev_wick_high) <= tolerance) |
            (abs(body_high - prev_wick_high) <= tolerance) |
            (abs(df["High"] - prev_body_high) <= tolerance) |
            (abs(body_high - prev_body_high) <= tolerance)
        )
    ).astype(int)
...
def EQL(df, tolerance=1):
    ...
    prev_wick_low = df["Low"].where(swing_low).ffill().shift(1)
    prev_body_low = body_low.where(body_swing_low).ffill().shift(1)

    return (
        (swing_low | body_swing_low) &
        (
            (abs(df["Low"] - prev_wick_low) <= tolerance) |
            (abs(body_low - prev_wick_low) <= tolerance) |
            (abs(df["Low"] - prev_body_low) <= tolerance) |
            (abs(body_low - prev_body_low) <= tolerance)
        )
    ).astype(int)
...
def add_indicators(df):
    ...
    df = df[["Open", "High", "Low", "Close",
            "k", "k_smooth", "adx", "+di", "-di", "EMA7", "EMA21", "EMA721_DIFF", "EMA7_Slope",
...
def train_bot(symbol="XAUUSD"):
    ...
    for i in range(SEQ_LEN, len(df)):
        ...
        if in_position:
            ...
            if action == 3:
                if position_type == "long":
                    pnl = (current_price - entry_price) * position_multiplier / PIP_VALUE - SPREAD_AND_COMMISSION * position_multiplier
                    trade_closed = True
                elif position_type == "short":
                    pnl = (entry_price - current_price) * position_multiplier / PIP_VALUE - SPREAD_AND_COMMISSION * position_multiplier
                    trade_closed = True

            if action == 0 and position_type == "long" and (current_price - entry_price) / PIP_VALUE >= 10:
                sl_price = entry_price
            elif action == 0 and position_type == "short" and (entry_price - current_price) / PIP_VALUE >= 10:
                sl_price = entry_price
                trade_closed = True
            """
...
def test_bot(symbol="XAUUSD"):
    ...
    while True:
        ...
        if current_m5 != last_m5:
            ...
            open_pos = mt5_open_positions(symbol)
